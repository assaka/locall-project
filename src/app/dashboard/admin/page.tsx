    ...
    <Container maxWidth="xl" sx={{ py: 4 }}>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        <Grid item xs={12} sm={6} md={3}>
          <MetricCard
            title="System Health"
            value={systemHealth ? `${systemHealth.uptime}%` : '--'}
            subtitle="Uptime"
            icon={<MonitorIcon />}
            color="#1976d2"
        ...
        <Grid item xs={12} sm={6} md={3}>
          <MetricCard
            title="Active Users"
            value={userMetrics ? userMetrics.active_users.toLocaleString() : '--'}
            subtitle={userMetrics ? `${userMetrics.total_users.toLocaleString()} total` : '--'}
            icon={<PeopleIcon />}
            color="#2e7d32"
            trend={userMetrics ? { value: userMetrics.user_growth_rate, direction: 'up' } : undefined}
            loading={loading}
          />
        </Grid>
        <Grid item xs={12} sm={6} md={3}>
          <MetricCard
            title="API Calls"
            value={systemMetrics ? systemMetrics.api_calls.toLocaleString() : '--'}
            subtitle="Today"
            icon={<DataUsageIcon />}
            color="#ed6c02"
        ...
        <Grid item xs={12} sm={6} md={3}>
          <MetricCard
            title="Storage Used"
            value={systemMetrics ? `${systemMetrics.storage_used}GB` : '--'}
          ...
          {selectedTab === 0 && (
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} lg={8}>
                ...
                <Card sx={{ p: 3, borderRadius: 2, mb: 3 }}>
                  ...
                  <Box sx={{ mt: 2, textAlign: 'right' }}>
                    <Button 
                      variant="outlined" 
                      size="small"
                      onClick={() => window.open('/dashboard/admin?tab=fraud', '_blank')}
                    >
                      View Details
                    </Button>
                  </Box>
                ...
                {/* Call Recording & Sentiment Summary */}
                <Card sx={{ p: 3, borderRadius: 2, mb: 3 }}>
                  <Typography variant="h6" sx={{ mb: 3, fontWeight: 600 }}>
                    🎙️ Call Analytics Summary (24h)
                  </Typography>
                  <Grid container spacing={3}>
                    <Grid item xs={12} sm={6} md={3}>
                      <Box sx={{ textAlign: 'center', p: 2, backgroundColor: '#f5f5f5', borderRadius: 2 }}>
                  ...
                  <Box sx={{ mt: 2, textAlign: 'right' }}>
                    <Button 
                      variant="outlined" 
                      size="small"
                      onClick={() => window.open('/dashboard/admin?tab=calls', '_blank')}
                    >
                      View Details
                    </Button>
                  </Box>
